...
def contact(request):
    ...
    if request.method == 'POST':  # если пост запрос был
        ...
        if message_form.is_valid():  # если форма заполнена правильно
            message_tg = f'{request.POST["name"]}\n{request.POST["phone"]}\n{request.POST["message"]}'  # форматированнная строка для тг
            bot = telebot.TeleBot('2093881054:AAE0wrH3JsZiUuQ8b70gfhzpXxZLunGEzHc')     # подключение к боту через токен
            bot.send_message(1785433786, message_tg)    # отправка в тг по id сообщения message_tg
